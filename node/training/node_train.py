...
# --- Training Loop with the Riemannian MSE using Optuna Strategy
def train_node_energy_goal_imitation_riemannianmse_optuna(model, vae, train_loader, val_loader, node_cfg, device='cpu', trial=None):
    # --- 1. Dynamic Config Extraction ---
    METRIC_SCALE_ENERGY = node_cfg['training']['metric_scale_energy']
    SAFE_BASELINE = node_cfg['training']['safe_baseline']
    lr = node_cfg['training'].get('learning_rate', 1e-4)
